...
RUN pip3 install numpy==1.23.0 llvmlite numba tensorboardX easydict pyyaml scikit-image tqdm SharedArray open3d mayavi av2 kornia pyquaternion opencv-python==4.5.4.58
RUN pip3 install spconv-cu118

RUN git clone git@github.com:MarvinKlemp/a2rl_OpenPCDet.git

WORKDIR OpenPCDet

RUN python3 setup.py develop
RUN pip install nuscenes-devkit==1.0.5 kornia==0.5.8
